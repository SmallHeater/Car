//
//  SHPictureSelectionController.m
...
@interface SHPictureSelectionController ()
...
@implementation SHPictureSelectionController

#pragma mark  ----  生命周期函数
+(SHPictureSelectionController *)sharedManager{

    static SHPictureSelectionController * controller = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        controller = [[SHPictureSelectionController alloc] init];
    });
    return controller;
}
...
- (void)loadAllPhoto:(void(^)(NSMutableArray *arr))result
{
     ...
     __weak SHPictureSelectionController *weakSelf = self;
    
    if (self.configurationModel.sourceType == SourceImage) {
    ...
    else if (self.configurationModel.sourceType == SourceVideo){
...
-(void)clearMemary{
    ...
    [self.shAssetModelArray removeAllObjects];
    self.shAssetModelArray = nil;
    [self.groupArrays removeAllObjects];
    self.groupArrays = nil;
    self.cameraModel = nil;
    self.configurationModel = nil;
}
